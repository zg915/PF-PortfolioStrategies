...
def CMSV(observation, L = 1_000_000, δ = 0.98):
    """
    Constant Parameters
    """
    ...
    μ̄ = np.zeros((T, L)) # latent μ, shape (T, L)
    p_μ̄ = np.zeros((T, L)) # particles of latent μ, shape (T, L)
    ...
    μ̄[0] = U(-5, 5, L) # shape (L, 1)
    x̄[0] = U(-1, 5, L) # shape (L, 1)
    ϕ_x[0] = 2 * B(20, 1.5, L) - 1 # shape (L, 1)
    σ_x[0] = U(0, 2, L) # shape (L, 1)

    
    x[0] = N(x̄[0], (σ_x[0] ** 2) / (1 - ϕ_x[0] ** 2))

    ϵ[0] = N(0, 1, L)
    y[0] = observation_function(μ̄[0], x[0], ϵ[0])
    ...
    for t in tqdm(range(1, T, 1)):
        ...
        ϵ[t] = N(0, 1, L)
        y[t] = observation_function(p_μ̄[t], p_x[t], ϵ[t])


        # calculate likelihood with Gaussian likelihood
        α[t] = observation_likelihood(observation[t], p_μ̄[t], p_x[t])
    ...
    """AD 2"""
    AD2 = np.exp(- np.median(p_x, axis=1)) * ((observation - np.median(p_μ̄, axis=1)) ** 2)
...
if __name__ == "__main__":
    ...
    dates = returns.index
    y, AD1, AD2, AD3 = CMSV(observation, L = 1_000)
    print(AD1)
    print(AD2)
    print(AD3)
