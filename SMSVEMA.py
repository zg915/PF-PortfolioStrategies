...
def SMSVEMA(observation, L = 1_000_000, δ = 0.98, β = 0.8):
    """Constant Parameters"""
    ...
    α_μ_parameter = np.full(L, np.mean(Y[:24])) # mean of first 2 year return
    ϕ_μ =  np.full(L, 1 - β)# shape (L, 1)
    σ_μ = U(0, calculate_σ_μ(α_μ_parameter, Y_train, β), L) # shape (L, 1)
    ...
    μ[0] = N(α_μ_parameter, (σ_μ ** 2) / (1 - ϕ_μ ** 2))
    ...
    for t in tqdm(range(1, T, 1)):
        ...
        p_μ[t] = μ_transition_function(observation[t-1], ϕ_μ, μ[t-1])
...
if __name__ == "__main__":
    ...
    dates = returns.index
    y, AD1, AD2, AD3 = SMSVEMA(observation, L = 1_000_000, β = 0.2)
    print(AD1)
    print(AD2)
    print(AD3)
